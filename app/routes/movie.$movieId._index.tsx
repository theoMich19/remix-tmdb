...
import { loader } from "~/domains/media/controllers/movie-details"
import { MovieRecommendations } from "~/domains/media/types/movie"
import { getFormatDate } from "~/utils/date"

export { loader }

export default function MediaMovieDetails() {
  const { movieDetails, movieRecommendations, moviesImagesList } =
    ...
    <main className="w-full h-full gap-8 sm:items-center sm:justify-center bg-black text-white overflow-scroll  bg-galaxy bg-cover bg-center m-0">
      ...
      <section className="flex flex-col md:flex-row mx-auto w-full md:w-[80vw]  md:mt-4">
        ...
        <div className="w-full mx-4 md:ml-8 space-y-4 bg-black bg-opacity-50 p-4 rounded-lg">
          ...
          <div className="space-y-4 text-gray-300">
            <span>Sortie : {getFormatDate(movieDetails.release_date)}</span>
      ...
      <section className="flex flex-col mx-auto w-full md:w-[80vw]  md:mt-4">
        ...
        <Carousel className="w-full md:w-[90vw] mx-auto mt-8">
          <CarouselContent>
            {movieRecommendations.results.map((item: MovieRecommendations) => {
              return (
                <CarouselItem
                  key={item.id}
      ...
      <section className="flex flex-col mx-auto w-full md:w-[80vw]  md:mt-4 overflow-hidden">
        <h4>Gallerie photos</h4>
        <div className="flex flex-wrap justify-center gap-2">
          {moviesImagesList.backdrops.map((item) => {
            return (
              <div
                key={item.file_path}
